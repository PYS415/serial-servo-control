...
char Serial_RxPacket1[100];				//定义接收数据包数组，数据包格式"@MSG\r\n"
...
void Serial_SendNumber1(uint32_t Number, uint8_t Length)
{
	...
	{
		Serial_SendByte1(Number / Serial_Pow1(10, Length - i - 1) % 10 + '0');	//依次调用Serial_SendByte发送每位数字
	}
}
...
void USART2_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART2, USART_IT_RXNE) == SET)	//判断是否是USART1的接收事件触发的中断
	{
		...
		if (RxState == 0)
		{
			if (RxData == '@' && Serial_RxFlag1 == 0)		//如果数据确实是包头，并且上一个数据包已处理完毕
			{
				RxState = 1;			//置下一个状态
		...
		else if (RxState == 2)
		{
			...
			{
				RxState = 0;			//状态归0
				Serial_RxPacket1[pRxPacket] = '\0';			//将收到的字符数据包添加一个字符串结束标志
