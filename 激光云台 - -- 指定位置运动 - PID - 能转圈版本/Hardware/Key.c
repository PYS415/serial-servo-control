...
//	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);	//开启GPIOA的时钟
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10 | GPIO_Pin_11 | GPIO_Pin_0 | GPIO_Pin_1;
...
{
	if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_11) == 0)		//如果PB10引脚电平为0
	{
		return 1;		//直接返回键码1
	}
	if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_10) == 0)		//如果PB11引脚电平为0
	{
		return 2;		//直接返回键码2
	}
	if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_1) == 0)		//如果PA11引脚电平为0
	{
		return 3;		//直接返回键码3
	}
	if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_0) == 0)		//如果PA12引脚电平为0
	{
		return 4;		//直接返回键码4
	}
